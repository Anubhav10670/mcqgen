...
function App() {
  ...
  const generateQuestions = async (text: string, numQuestions: number) => {
    ...
    const prompt = `Generate ${numQuestions} multiple-choice quiz based on the following text. The output should be strictly in JSON format, containing only the questions, options, and correct answers (avoid activities if there to make mcqs and make useful mcqs with value). Do not include any additional remarks, reasoning, or explanation. Format the response as a JSON array where each question object has the format: {"question": "...", "options": ["...", "...", "...", "..."], "correctAnswer": "..."}\n\nText: ${text}`;

    try {
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Authorization': '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',
